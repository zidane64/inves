# main.py - Sistem Investasi Real-Time untuk Railway
...
class User:
    def __init__(self, username: str):
        ...
        self.saldo_tunai = 1000000.0
        self.saldo_unit = 0.0

class Transaksi:
    def __init__(self, username, jenis, jumlah_unit, harga, fee):
        self.username = username
        self.jenis = jenis
...
class SistemInvestasi:
    ...
    def update_harga(self):
        """Update harga dengan perubahan halus"""
    ...
    def beli(self, username: str, jumlah_unit: float):
        ...
        if user.saldo_tunai >= total_dengan_fee:
            ...
            raise HTTPException(400, f"Saldo tidak cukup")
    ...
    def jual(self, username: str, jumlah_unit: float):
        ...
        if user.saldo_unit < jumlah_unit:
            raise HTTPException(400, f"Unit tidak cukup")
    ...
    def get_portofolio(self, username: str):
        ...
        user_transaksi = [t for t in self.transaksi_history if t.username == username]
...
# Background task untuk update harga setiap 4 jam
async def background_updater():
    while True:
        await asyncio.sleep(4 * 3600)  # 4 jam
...
async def lifespan(app: FastAPI):
    ...
    print("✅ Sistem investasi berjalan dengan update harga setiap 4 jam")
    yield
    # Cleanup jika perlu
...
def root():
    return {
        ...
        "last_update": sistem.last_update.isoformat(),
        "endpoints": {
...
def get_harga():
    return {
        ...
        "riwayat_harga": [
            {"waktu": h["waktu"].isoformat(), "harga": h["harga"]} 
            for h in sistem.riwayat_harga[-30:]
        ]
    }
    ...
    <body>
        <div class="container">
            ...
            <div class="card">
                ...
                <p style="margin-top: 10px; font-size: 12px; color: #666;">*Harga update otomatis setiap 4 jam dengan perubahan -1.5% hingga +1.5%</p>
            </div>
        </div>
        ...
        <script>
            ...
            async function beli() {
                ...
                if(!user || !jumlah) return alert('Isi semua field');
                ...
                if(data.status === 'sukses') loadHarga();
            }
            ...
            async function lihatPortofolio() {
                ...
                if(data.total_dividen > 0) html += '<p class="profit">✅ Total dividen: Rp ' + data.total_dividen.toLocaleString() + '</p>';
                document.getElementById('portResult').innerHTML = html;
            }
            
            loadHarga();
            setInterval(loadHarga, 30000);
        </script>
    </body>
    </html>
    """
